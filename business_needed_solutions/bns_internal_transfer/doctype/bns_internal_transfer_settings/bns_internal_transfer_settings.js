...
frappe.ui.form.on("BNS Internal Transfer Settings", {
	refresh: function (frm) {
		...
		frm.add_custom_button(__("Bulk Convert to BNS Internal"), function () {
			...
			const fields = [
				...
				{ fieldtype: "Section Break", label: __("Preview") },
				{
					...
					options:
						'<div id="preview-content" style="padding: 10px; background: #f0f0f0; border-radius: 4px; min-height: 100px;">' +
					...
					callback: function (r) {
						if (!r.exc && r.message) {
							...
							if (preview.total_count === 0) {
								frappe.show_alert({ message: __("No documents found to convert"), indicator: "orange" });
							} else {
								frappe.show_alert({
			...
			const d = new frappe.ui.Dialog({
				...
				primary_action(values) {
					if (!values.from_date) {
						frappe.msgprint({ title: __("Validation Error"), message: __("From Date is required"), indicator: "red" });
						return;
					}
			...
			setTimeout(function () {
				if (d.fields_dict.from_date) {
					d.fields_dict.from_date.$input.on("change", function () {
						if (d.fields_dict.from_date.get_value()) updatePreview(d);
					});
				}
				if (d.fields_dict.force) {
					d.fields_dict.force.$input.on("change", function () {
						if (d.fields_dict.from_date && d.fields_dict.from_date.get_value()) updatePreview(d);
					});
				}
			}, 100);
		}, __("Actions"));

		// Filters for account fields - restrict to relevant account types
		frm.set_query("stock_in_transit_account", function () {
			return {
				filters: {
					is_group: 0,
					root_type: "Asset"
		...
		frm.set_query("internal_sales_account", function () {
			return {
				filters: {
					is_group: 0,
					root_type: "Income"
		...
		frm.set_query("internal_purchase_account", function () {
			return {
				filters: {
					is_group: 0,
					root_type: "Expense"
		...
		frm.set_query("internal_branch_debtor_account", function () {
			return {
				filters: {
					is_group: 0,
