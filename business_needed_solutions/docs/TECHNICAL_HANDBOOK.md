...
|--------|---------------|-----------|
| **BNS Counter Repack (new module)** — a Sales Invoice row ticked "Repack from Bulk" posts a Repack at submit: the whole bulk lot out, the sold grade in at net rate × (1 − margin %) from the latest effective `BNS Counter Repack Rule`, the rest of the lot back in as the Stock Value Conservation plug; cancelled with the invoice. Bulk items must be Moving Average; a grade must hold no stock; the bill is blocked when the lot cannot cover the margin cost. Full design: root `technical_handbook.md`, 2026-09-19 | `bns_counter_repack/` (new), `modules.txt`, `hooks.py`, `fixtures/custom_field.json` | No way to book counter sales of grades sorted out of a single averaged bulk hing item at a known margin without posting to Stock Adjustment |
| **Stock value conservation (Repack / Manufacture)** — one untyped incoming line (the plug) takes `(Σ outgoing basic − Σ other incoming basic) / qty` at submit and on every Repost Item Valuation; additional costs land on the plug; submit guard `guard_stock_adjustment` blocks a net Stock Adjustment balance on Stock Entry / PR / PI / SI / DN (Stock Reconciliation exempt); `verify_after_repost` logs any residual after a repost. Company-wise, fail-closed, off by default (BNS Settings → Manufacturing → Stock Value Conservation). Full design: root `technical_handbook.md`, 2026-09-19 | `overrides/stock_value_conservation.py` (new), `overrides/stock_entry_component_qty_variance.py`, `doctype/bns_settings/*`, `doctype/bns_stock_value_conservation_rule/` (new), `hooks.py`, `fixtures/custom_field.json`, `tests/test_stock_value_conservation.py` (new) | Frozen typed incoming rates on multi-output Repack / Manufacture leaking incoming − outgoing to Stock Adjustment after every backdated repost |
| **DN→SI→PI transfer-rate chain fallback** — `_build_si_rate_maps_for_pi` now backfills zero-rate SI items from linked DN Item `incoming_rate` and DN SLE via new `_resolve_rate_from_dn_chain`; `_resolve_pi_item_transfer_rate_extras` adds DN SLE as final fallback (step 5) via new `_get_rate_from_dn_stock_ledger`; `apply_internal_pi_transfer_rates_from_si` and `_sync_pi_item_transfer_rate_from_si` now build and pass `si_dn_map` through the chain | `bns_branch_accounting/utils.py` | PI transfer-rate missing when SI `incoming_rate` was 0 because DN→SI rate propagation hadn't run and non-update-stock SI has no SLE |
| **SI-PI amount tolerance from settings** — `validate_si_pi_items_match` now accepts `amount_tolerance` parameter; taxable value, grand total, and tax comparisons ignore differences below threshold; parity validator reads `si_pi_amount_tolerance` from `BNS Branch Accounting Settings`; new Currency field added to settings doctype | `bns_branch_accounting/utils.py`, `bns_branch_accounting/doctype/bns_branch_accounting_settings/bns_branch_accounting_settings.json` | Minor rounding differences between SI and PI blocking submit despite being within acceptable range |
| **Submit-time SI-PI parity enforcement** — new `validate_internal_purchase_invoice_si_parity` runs on PI `before_submit` for BNS internal different-GSTIN flows; resolves source SI via canonical `_resolve_si_name_for_internal_pi` (header ref, bill_no, PR chain), then calls `validate_si_pi_items_match(check_all=True)` and throws a blocking error with item/qty/taxable/total/tax mismatches | `bns_branch_accounting/utils.py`, `hooks.py` | Internal PI submitted without error despite diverging items/amounts/taxes from source Sales Invoice |
| **Canonical SI resolver extended to PR chain** — `_resolve_si_name_for_internal_pi` now checks `Purchase Receipt.bns_inter_company_reference` when header ref and bill_no are absent, covering SI->PR->PI flows | `bns_branch_accounting/utils.py` | PR-linked PIs not recognized as SI-backed when PI header lacked direct SI reference |
| **Unified PI internal detection** — `_is_bns_internal_purchase_invoice_from_si` now delegates to `_resolve_si_name_for_internal_pi` instead of duplicating ref/bill_no/PR logic inline | `bns_branch_accounting/utils.py` | Inconsistent SI detection between status assignment and repost reassertion |
| **PI status Overdue drift prevention** — `update_purchase_invoice_status_for_bns_internal` now uses canonical detection helpers and re-asserts `BNS Internally Transferred` after repost triggers, preventing ERPNext outstanding logic from overwriting status to Overdue | `bns_branch_accounting/utils.py` | PI status set to Overdue instead of BNS Internally Transferred after submit + repost |
| **PI transfer-rate auto-fill before validate** — `validate_internal_purchase_invoice_transfer_rate` now calls `apply_internal_pi_transfer_rates_from_si` first: fills `bns_transfer_rate` from SI item link, PR `pr_detail`, item_code/qty bucket match (same rules as `link_si_pi`), then `Stock Ledger Entry` rows for the source Sales Invoice when SI `incoming_rate` / links are missing; optional backfill of `sales_invoice_item`; `_sync_pi_item_transfer_rate_from_si` uses the same fallbacks | `bns_branch_accounting/utils.py` | Internal update-stock PI blocked on Save with “transfer-rate is missing” because sync only ran on submit and many rows had no `sales_invoice_item` / PR rate |
...
> **See also:** the same override class (`BNSStockEntry`) now hosts Stock Value Conservation — `validate_repack_entry`, `set_basic_rate`, `distribute_additional_costs` and `get_finished_item_row` are overridden and delegate to `overrides/stock_value_conservation.py`. Root `technical_handbook.md`, entry 2026-09-19.
