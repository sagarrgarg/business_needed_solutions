...
def get_gl_entries(filters, accounting_dimensions):
    ...
    # Fetch additional details for the party column and type
	for gle in gl_entries:
		if not gle.get("party") or not gle.get("party_type"):
			party_type, party_name = get_party_details_from_against(gle.get("against"))
			gle["party"] = party_name
...
def get_party_details_from_against(against):
    """
    Check the against account and return both the party type and party name
    if it matches a Customer, Supplier, Shareholder, or Employee.
    """
    if not against:
        return None, None

    # Check each party type
    party_type_mapping = {
    ...
    for doctype, name_field in party_type_mapping.items():
        name = frappe.db.get_value(doctype, {"name": against}, name_field)
        if name:
            return doctype, name  # Return the matched type and name
