...
def execute(filters=None):
	...
	for party, row in main_dict.items():
		...
		if is_common_party:
			...
			# guard at line ~234). After netting, if outstanding went negative the
			# balance is receivable — skip from this Payable report; the Receivable
		...
		# Standalone suppliers with negative outstanding (pure advance /
		# debit notes > invoices) are kept - they have no customer mirror
