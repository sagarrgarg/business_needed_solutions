"""
BNS Branch Accounting - Utility Functions

Core engine for branch accounting: DN/PR, SI/PI linking, conversion, validation,
bulk operations, status updates, and GST-aware billing calculations.
"""
...
def make_bns_internal_purchase_invoice(source_name: str, target_doc: Optional[Dict] = None) -> Dict:
    ...
    try:
        ...
        # Get representing company
        represents_company = _get_representing_company(si.customer)
...
def validate_sales_invoice_company_address_from_dn(doc, method: Optional[str] = None) -> None:
    """
    For internal transfer with different GSTIN: when Sales Invoice is created from
    a Delivery Note, Company Address and Company GSTIN must match the DN exactly.
    Do not allow save if they differ (prevents wrong company address on first save).
    
    Applies when:
    - SI has items with delivery_note (picked from DN)
    - DN's customer is BNS internal
    - DN has different GSTIN (billing_address_gstin != company_gstin = inter-state flow)
    """
    if not doc.get("items"):
        return
    # Get delivery note(s) from items
    dn_names = {item.get("delivery_note") for item in doc.items if item.get("delivery_note")}
    if not dn_names:
        return
    try:
        for dn_name in dn_names:
            if not dn_name or not frappe.db.exists("Delivery Note", dn_name):
                continue
            dn = frappe.get_doc("Delivery Note", dn_name)
            # Check if BNS internal customer
            is_bns_internal = dn.get("is_bns_internal_customer") or False
            if not is_bns_internal:
                customer_internal = frappe.db.get_value("Customer", dn.customer, "is_bns_internal_customer")
                if not customer_internal:
                    continue
            # Check if different GSTIN (inter-state transfer)
            billing_gstin = (dn.get("billing_address_gstin") or "").strip()
            company_gstin_dn = (dn.get("company_gstin") or "").strip()
            if not billing_gstin or not company_gstin_dn or billing_gstin == company_gstin_dn:
                continue
            # This DN is internal + different GSTIN — SI must match DN's company details
            si_company_address = (doc.get("company_address") or "").strip()
            si_company_gstin = (doc.get("company_gstin") or "").strip()
            dn_company_address = (dn.get("company_address") or "").strip()
            if si_company_address != dn_company_address:
                frappe.throw(
                    _(
                        "For internal transfer with different GSTIN, Sales Invoice created from Delivery Note {0} "
                        "must use the same Company Address as the Delivery Note. "
                        "Expected: {1}, got: {2}"
                    ).format(
                        get_link_to_form("Delivery Note", dn_name),
                        bold(dn_company_address or _("(from Delivery Note)")),
                        bold(si_company_address or _("(empty)"))
                    ),
                    title=_("Company Address Mismatch")
                )
            if si_company_gstin != company_gstin_dn:
                frappe.throw(
                    _(
                        "For internal transfer with different GSTIN, Sales Invoice created from Delivery Note {0} "
                        "must use the same Company GSTIN as the Delivery Note. "
                        "Expected: {1}, got: {2}"
                    ).format(
                        get_link_to_form("Delivery Note", dn_name),
                        bold(company_gstin_dn or _("(from Delivery Note)")),
                        bold(si_company_gstin or _("(empty)"))
                    ),
                    title=_("Company GSTIN Mismatch")
                )
    except frappe.ValidationError:
        raise
    except Exception as e:
        logger.error(f"Error validating Sales Invoice company address from DN: {str(e)}")
        pass
