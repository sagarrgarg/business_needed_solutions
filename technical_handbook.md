...
- **Settings:** BNS Branch Accounting Settings – `stock_in_transit_account`, `internal_sales_transfer_account` (GST / Inter-State, used by SI rewrite), `internal_sales_non_gst_account` (Non-GST / Same-GSTIN, used by DN rewrite; falls back to the GST field if blank), `internal_purchase_transfer_account` (GST / Inter-State, used by PI rewrite), `internal_purchase_non_gst_account` (Non-GST / Same-GSTIN, used by PR rewrite; falls back to the GST field if blank), `internal_branch_debtor_account`, `internal_branch_creditor_account`, `enable_internal_dn_ewaybill`, `internal_transfer_cutoff_fy` (Link to Fiscal Year), `accounting_rewrite_cutoff_fy` (Link to Fiscal Year). Old `internal_validation_cutoff_date` is hidden/deprecated. Splitting the DN/PR (Non-GST) account from the SI/PI (GST) account lets Trial Balance natively separate internal-DN, internal-SI, and external sales — and `external + internal-SI` then reconciles directly to GSTR-1 (mirror for GSTR-2 on the purchase side). Sites that point both fields at the same account keep current behavior.
