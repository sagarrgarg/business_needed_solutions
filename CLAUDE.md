...
- **BNS Counter Repack** — a Sales Invoice row ticked "Repack from Bulk" sorts the billed grade out of a bulk lot at submit (Repack: whole lot out, grade in at margin cost, rest of lot back as the Stock Value Conservation plug). Rules in `BNS Counter Repack Rule`; see `bns_counter_repack/counter_repack.py`
